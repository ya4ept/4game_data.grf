	...
	if timeInfo == nil then
		...
		if StateIconList ~= nil then
			...
			if obj ~= nil then
				...
				resultDesInfos[key] = { haveTimeLimit, posTimeLimitStr }
...
resultDesInfos = SetDefaultDesInfo
...
MakeBuffIconInfoFile = function()
	...
	if makeFile ~= nil then
		makeFile:write("data\\StateIconInfo_n.lua", "StateIconList = {}\n\n")
		for k, v in pairs(EFST_IDs) do
			filename, msg = c_GetEFSTMsg(v)
			if filename ~= "" and filename ~= nil then
				makeFile:write(v, string.format("-- %d, %s\n", v, filename))
				makeFile:write(v, string.format("StateIconList[EFST_IDs.%s] =\n", k))
				makeFile:write(v, "{\n")
				makeFile:write(v, "\tdescript = \n")
				makeFile:write(v, "\t{\n")
				if msg ~= "" and msg ~= nil then
					makeFile:write(v, string.format("\t\t{ \"%s\", },\n", msg))
				end
				makeFile:write(v, "\t}\n")
				makeFile:write(v, "}\n\n")
			end
